...
async function loadRooms() {
    try {
        ...
        // ID du salon épinglé
        const PINNED_ROOM_ID = 'fc4abfefdfad502c';
        
        // Séparer le salon épinglé des autres
        const pinnedRoom = data.rooms.find(room => room.id === PINNED_ROOM_ID);
        const otherRooms = data.rooms.filter(room => room.id !== PINNED_ROOM_ID);
        
        // Trier les autres salles par dernière activité - du plus récent au plus ancien
        otherRooms.sort((a, b) => new Date(b.last_activity) - new Date(a.last_activity));
        
        // Afficher d'abord le salon épinglé
        if (pinnedRoom) {
            const pinnedElement = createRoomElement(pinnedRoom, true);
            roomsList.appendChild(pinnedElement);
            
            // Ajouter un séparateur si d'autres salles existent
            if (otherRooms.length > 0) {
                const separator = document.createElement('div');
                separator.className = 'room-separator';
                separator.innerHTML = '<hr><small>Autres salons</small>';
                roomsList.appendChild(separator);
            }
        }
        
        // Afficher les autres salles triées
...
// Fonction helper pour créer un élément de salon
function createRoomElement(room, isPinned) {
    ...
    roomElement.className = `room-item ${roomClass} ${isPinned ? 'pinned' : ''}`;
    
    roomElement.innerHTML = `
        ${isPinned ? '<div class="pinned-badge">📌 Épinglé</div>' : ''}
